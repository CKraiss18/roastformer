...
### The Roasting Process: Why Temperature Control Matters

Roasting transforms green coffee through three critical phases, each requiring precise temperature control:

**1. Drying Phase (0-4 minutes, 300-380°F)**
- Beans lose 12-15% moisture content
- Temperature drops after charge, then recovers (turning point ~320°F)
- Endothermic (beans absorb heat)
- Smell: Grassy, hay-like → toasted grain

**2. Maillard Phase (4-8 minutes, 380-395°F)**
- Browning reactions: sugars + amino acids → 800+ flavor compounds
- Exothermic (beans release heat)
- Color: Yellow-tan → light brown
- Smell: Bread, caramel, toast

**3. Development Phase (8-12 minutes, 395-430°F)**
- Post-first crack (beans audibly pop at ~385°F when cell structure breaks)
- Final 60-90 seconds determine roast level
- **Critical**: 10°F difference = light vs medium roast
- Smell: Coffee, chocolate, nuts, caramel

**The Challenge**: A single 5°F mistake in development can ruin a $50-200 batch. Roasters must balance:
- Time (too fast = underdeveloped, too slow = baked)
- Temperature (too hot = scorched, too cool = grassy)
- Rate-of-rise (must slow approaching first crack)

**The Opportunity**: Data-driven starting profiles reduce trial-and-error from 10-20 roasts to 2-3 validation roasts.
...
### How Cross-Attention Learns: Dynamic Feature Weighting

The transformer uses **cross-attention** to learn which conditioning features matter at different points in the roast:

**Mechanism**: At each timestep t, the model "queries" all conditioning features:
```
Query = temperature_state[t]  # "What should temperature be next?"
Keys = [origin, process, flavors, altitude, ...]  # All conditioning features
Attention_weights = softmax(Query · Keys / √d_k)  # Which features are relevant now?
```

**What We Hypothesize the Model Learns** (based on roasting physics):

| Roast Phase | Time | Temp Range | High-Attention Features | Why? |
|-------------|------|------------|------------------------|------|
| **Early Drying** | 0-3 min | 300-350°F | Altitude, Bean density | Heat transfer rate matters most |
| **Maillard** | 4-8 min | 350-385°F | Process (Washed/Natural) | Process affects Maillard development |
| **First Crack** | ~8 min | 385-395°F | Altitude, Bean density | Crack timing varies with density |
| **Development** | 9-12 min | 395-430°F | **Flavors**, Roast level | Flavor development is critical |

**Key Insight**: The model doesn't just use all features equally—it learns to **dynamically weight** which bean characteristics and flavor targets matter at each moment, matching how expert roasters think about different roast phases.

**Example**: When predicting temperature at first crack (~8 min, 385°F), the model likely attends strongly to:
- Altitude (40% attention): High-altitude beans crack earlier
- Bean density (30%): Dense beans need more heat
- Flavors (20%): Berry flavors need careful first-crack control
- Roast level (10%): Target determines post-crack trajectory
...
### Live Demo: Interactive Profile Generation

**Quick Command-Line Demo**:
```bash
# Generate Ethiopian berry/floral profile
...
# Generate profile with custom inputs
profile = model.generate(
    origin='Ethiopia',
    process='Washed',
    flavors=['berries', 'floral', 'citrus'],
...
plot_profile(profile, title="Ethiopian Berry/Floral")
```

**Demo 2: Flavor Comparison** (Shows +14% improvement in action)
```python
# Same coffee, different flavor targets
profile_berry = model.generate(..., flavors=['berries', 'citrus'])
profile_chocolate = model.generate(..., flavors=['chocolate', 'caramel'])

# Side-by-side comparison
compare_profiles({
    'Berry-focused': profile_berry,
    'Chocolate-focused': profile_chocolate
})
# → Different temperature trajectories for different flavor goals!
```

**Demo 3: Physics Validation** (Shows the 0% compliance challenge)
```python
from src.utils.validation import validate_physics

results = validate_physics(profile)
print(f"✅ Smooth transitions: {results['smooth']:.1%}")      # 98.7%
print(f"⚠️  Bounded heating: {results['bounded_ror']:.1%}")  # 28.8%
print(f"❌ Monotonicity: {results['monotonic']:.1%}")        # 0.0%
# → Reveals exposure bias challenge discussed in Methodology
```

**Demo 4: Diverse Origins** (Shows learned associations)
```python
# Compare how model treats different origins
origins = ['Ethiopia', 'Colombia', 'Guatemala', 'Kenya']
profiles = {o: model.generate(origin=o, ...) for o in origins}
...
To understand the practical impact, consider a typical specialty coffee roaster introducing new coffees:

**Current Workflow (Without RoastFormer)**:
- **Trial roasts**: 10-20 experimental roasts per coffee @ 15 min each = 2.5-5 hours
- **Bean cost**: $10-20/lb × 1-2 lbs per roast = $100-400 wasted
- **Labor cost**: 2.5-5 hours @ $25-50/hr = $62.50-250
- **Time to market**: 2-3 weeks of iteration
- **Total cost per coffee**: $200-650

**With RoastFormer (After Multi-Roaster Training)**:
- **Generation time**: 3-5 candidate profiles in <1 second
- **Validation roasts**: 2-3 roasts only = 30-60 minutes
- **Bean cost**: $20-60 (75% reduction)
- **Labor cost**: $12.50-50 (80% reduction)
- **Time to market**: 3-5 days (70% faster)
- **Total cost per coffee**: $50-150 (70-80% savings)

**Annual ROI for Small Roaster** (30 new coffees/year):
- **Time saved**: 75-150 hours/year
- **Cost saved**: $4,500-15,000/year
- **Additional value**: Faster time-to-market enables seasonal coffee launches
- **Payback period**: <1 month (assuming $500 software cost)

**Caveat**: These projections assume production-ready model (100% physics compliance) after implementing scheduled sampling + physics-informed losses + multi-roaster dataset. Current v1.0 is proof-of-concept only.
...
### Broader Applications: Methodology Generalization

The core approach—**conditional time-series generation with domain-specific physics constraints**—extends beyond coffee roasting to other manufacturing and food science domains:

| Domain | Problem | Conditioning Features | Physics Constraints | Similar Challenges |
|--------|---------|----------------------|--------------------|--------------------|
| **Coffee Roasting** | Generate roast profiles | Bean origin, flavor targets, altitude | Monotonicity, bounded heating rates | Small data, exposure bias ✅ |
| **Beer Brewing** | Fermentation temperature schedules | Yeast strain, hop varieties, target ABV | Temperature ranges, fermentation curves | Time-series, multi-modal features |
| **Cheese Aging** | Cave temperature/humidity profiles | Milk type, mold cultures, target texture | Humidity bounds, temperature stability | Long duration, sensory targets |
| **Bread Baking** | Oven temperature curves | Flour type, hydration, desired crust | Rising phases, crust formation | Phase transitions, final quality |
| **Chocolate Tempering** | Heating/cooling sequences | Cocoa origin, desired snap/shine | Crystal formation, phase transitions | Precise temperature control |
| **Kombucha Fermentation** | Temperature schedules | SCOBY strain, tea type, target sourness | pH ranges, temperature limits | Biological processes, flavor targets |
| **Metal Heat Treatment** | Annealing schedules | Alloy composition, target hardness | Cooling rates, phase diagrams | Material properties, physics-based |
| **Polymer Curing** | Temperature/pressure profiles | Resin type, fiber content, target strength | Cure kinetics, glass transition | Chemical reactions, final properties |

**Common Pattern**:
1. **Multi-modal conditioning**: Categorical (materials/ingredients) + continuous (properties/targets) + desired outcomes (flavors/textures)
2. **Sequential generation**: Autoregressive temperature/pressure/humidity profiles
3. **Physics constraints**: Domain-specific laws that generic models don't capture
4. **Small-data regime**: Limited expert examples (100-1000 samples typical)
5. **Evaluation gap**: Domain metrics (physics compliance, sensory quality) vs. generic metrics (RMSE)

**Key Insight**: RoastFormer's methodology—decoder-only transformer with cross-attention conditioning, physics-informed evaluation, and small-data regularization—provides a template for applying transformers to specialized manufacturing domains where:
- Data is scarce but expert knowledge is rich
- Physical laws constrain valid outputs
- Final quality depends on process trajectories, not just endpoints
- Multi-modal features (categorical + continuous + targets) drive outcomes

**Research Value**: Demonstrates that transformers can succeed in domains beyond NLP/vision when adapted with domain-specific evaluation and physics constraints, even with 100-1000 samples.
