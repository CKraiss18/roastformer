...
class OnyxDatasetBuilderV3:
    ...
    def extract_enhanced_metadata(self, driver, url):
        ...
        try:
            ...
            # CRITICAL FIX: Use separator to avoid character-by-character text extraction
            page_text = soup.get_text(separator=' ', strip=True)
            # Clean up multiple spaces
            page_text = re.sub(r'\s+', ' ', page_text)
            
            # Product name - try Selenium first for dynamically rendered content
            try:
                ...
                title = (soup.find('h1', class_=re.compile(r'product.*title', re.I)) or
                        ...
                        soup.find('h1'))
                if title:
                    product_name = title.get_text(separator=' ', strip=True)
            ...
            # Based on inspect: appears in <p> inside .stat-text, with text BEFORE label
            # Structure: <p>Washed\nPROCESS METHOD</p>
            
            # Method 1: Target the specific DOM structure (most reliable)
            try:
                # Look for .stat-text or .a-stat containing "PROCESS METHOD"
                ...
                for stat_div in stat_divs:
                    ...
                    if 'PROCESS METHOD' in stat_text.upper():
                        # Extract the text that comes BEFORE "PROCESS METHOD"
                        process_match = re.search(r'([A-Za-z\s-]+?)\s*PROCESS\s+METHOD', stat_text, re.I)
                        if process_match:
                            process_text = process_match.group(1).strip()
                            metadata['process'] = process_text.title()
                            break
            except Exception as e:
                pass
            
            # Method 2: Fallback to pattern matching if Method 1 fails
            if not metadata.get('process'):
                process_match = re.search(
                    r'([A-Za-z\s-]+?)\s*PROCESS\s+METHOD', 
                    page_text, 
                    re.I
                )
                if process_match:
                    ...
                    process_text = re.sub(r'\s*(TRADITIONAL|MODERN|HARVEST|VARIETY).*', '', process_text, flags=re.I)
                    metadata['process'] = process_text.title()
            
            # Method 3: Last resort keyword search
            if not metadata.get('process'):
            ...
            # Based on inspect: flavors are in <p class="tasting-notes kapra"> with <span class="note"> children
            # Structure: <p class="tasting-notes kapra"><span class="note">Berries</span>...</p>
            ...
            # Method 2: Fallback to class-based extraction if Method 1 fails
            if not metadata.get('flavor_notes_raw'):
                try:
                    # Look for any element with "tasting" or "note" in class name
                    note_elements = soup.find_all(['span', 'div', 'p'], class_=re.compile(r'note|tasting', re.I))
                    if note_elements:
                        # Filter to get actual flavor words (3-15 chars, not common words)
                        potential_flavors = []
                        for elem in note_elements:
                            text = elem.get_text(strip=True)
                            if 3 <= len(text) <= 15 and text.lower() not in ['the', 'and', 'or', 'note', 'notes']:
                                potential_flavors.append(text)
                        
                        if len(potential_flavors) >= 2:  # At least 2 flavors to be valid
                            metadata['flavor_notes_raw'] = ' '.join(potential_flavors)
                            metadata['flavor_notes_parsed'] = [f.upper() for f in potential_flavors]
                            metadata['flavor_categories'] = self._categorize_flavors([f.upper() for f in potential_flavors])
                except Exception as e:
                    pass
            
            # Method 3: Last resort - regex on page text for all-caps sequences
            if not metadata.get('flavor_notes_raw'):
                try:
                    # Look for main-title class that might contain flavors
                    main_title = soup.find(class_='main-title')
                    if main_title:
                        main_title_text = main_title.get_text(separator=' ', strip=True)
                        main_title_text = re.sub(r'\s+', ' ', main_title_text)
                        
                        # Find all-caps sequence (3+ words, 4+ chars each)
                        caps_pattern = r'\b([A-Z]{4,}(?:\s+[A-Z]{2,}){2,})\b'
                        caps_match = re.search(caps_pattern, main_title_text)
                        
                        if caps_match:
                            flavor_text = caps_match.group(1).strip()
                            metadata['flavor_notes_raw'] = flavor_text
                            flavor_words = flavor_text.split()
                            metadata['flavor_notes_parsed'] = flavor_words
                            metadata['flavor_categories'] = self._categorize_flavors(flavor_words)
                except Exception as e:
                    pass
    ...
    def scrape_roast_profile(self, url):
        ...
        try:
            ...
            # CRITICAL: Wait for amCharts to load FIRST, before any other operations
            print("    ⏳ Waiting for amCharts to fully load...")
            time.sleep(10)  # Give chart time to render
            ...
            try:
                ...
                # Still extract metadata even if no chart
            ...
            # Extract roast profile from amCharts IMMEDIATELY after wait
            # Don't do anything else that might cause delays
            chart_data_script = """
            ...
            # NOW extract metadata (chart data is safely captured)
            metadata = self.extract_enhanced_metadata(driver, url)
            profile = {
                'metadata': metadata,
                'roast_profile': {},
